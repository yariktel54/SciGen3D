...
def merge_tiles_srtm_30(tile_size):
    ...
    try:
        ...
        messagebox.showerror("Error", "Coordinate input format is incorrect. Enter latitude and longitude separated by commas.")
        return
    ...
    messagebox.showinfo("Success", "Tiles loaded successfully (SRTM 30)!")
...
def merge_tiles_srtm_90(tile_size):
    ...
    messagebox.showinfo("Success", "Tiles loaded successfully (SRTM 90)!")
...
def upload_custom_map():
    ...
    if file_path:
        try:
            ...
            messagebox.showinfo("Success", f"Map '{map_filename}' loaded successfully! Mode: {current_service}")
...
def create_3d_model():
    ...
    if combined_pixels is None:
        messagebox.showerror("Error", "Load tiles or custom map first!")
        return

    if current_service is None:
        messagebox.showerror("Error", "Select mode (SRTM 30 or SRTM 90) before generating 3D model.")
        return
    ...
    if current_service == "SRTM 30":
        ...
        messagebox.showerror("Error", "Unknown mode. Select SRTM 30 or SRTM 90.")
        return
    ...
    # Припустимо, що height_max_entry і height_x_val_entry - це Tkinter Entry
    try:
    ...
    # Продовження обробки height_map після перевірки
...
root = tk.Tk()
root.title("3D model of the Earth relief")

container = tk.Frame(root)
container.grid(row=0, column=0, padx=10, pady=10)

# Поле для введення координат
tk.Label(container, text="Coordinates (latitude, longitude):").grid(row=0, column=0, sticky=tk.W)
...
tk.Button(container, text="Load tiles", command=lambda: merge_tiles_srtm_30(int(tile_size_entry.get())) if service_var.get() == 1 else merge_tiles_srtm_90(int(tile_size_entry.get()))).grid(row=3, columnspan=2)
...
tk.Button(container, text="Load your own map", command=upload_custom_map).grid(row=6, columnspan=2)

# Кнопка для створення 3D моделі
tk.Button(container, text="Create a 3d model", command=create_3d_model).grid(row=7, columnspan=2)
...
tk.Label(additional_frame, text="Tile grid size (2 for 2x2, 3 for 3x3):").grid(row=3, column=0, sticky=tk.W)
tile_size_entry = tk.Entry(additional_frame)
tile_size_entry.grid(row=3, column=1, padx=10, pady=10)
tile_size_entry.insert(0, "1")

tk.Label(additional_frame, text="Choose a service to download:").grid(row=4, column=0, sticky=tk.W)
service_var = tk.IntVar(value=1)  # Початково вибрано SRTM 30
tk.Radiobutton(additional_frame, text="SRTM 30 (Black-White)", variable=service_var, value=1).grid(row=4, column=1, sticky=tk.W)
tk.Radiobutton(additional_frame, text="SRTM 90 (RGB)", variable=service_var, value=2).grid(row=4, column=2, sticky=tk.W)

# Параметри обмеження розміру
...
# Параметри для підсилення значень, нижчих за максимальну висоту
...
map_button = tk.Button(container, text="Open maps", command=open_google_maps)
